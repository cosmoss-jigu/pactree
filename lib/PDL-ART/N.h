...
//	static idxPtr allocAndWriteMemLog(OpStruct *oplog, int size, mappingTable *mtable);
...
//            smp_faa(&(pLock.lock),0b11);
        ...
        static void removeAndUnlock(N *node, uint8_t key, N *parentNode, uint8_t keyParent, ThreadInfo &threadInfo, bool &needRestart, OpStruct* oplog, uint32_t genId);
        ...
        static void insertGrow(curN *n, N *parentNode, uint8_t keyParent, uint8_t key, pptr<N> val, ThreadInfo &threadInfo, bool &needRestart, OpStruct* oplog, uint32_t genId);

        template<typename curN>
        static void insertCompact(curN *n, N *parentNode, uint8_t keyParent, uint8_t key, pptr<N> val, ThreadInfo &threadInfo, bool &needRestart, OpStruct* oplog, uint32_t genId);

        template<typename curN, typename smallerN>
        static void removeAndShrink(curN *n, N *parentNode, uint8_t keyParent, uint8_t key, ThreadInfo &threadInfo, bool &needRestart, OpStruct* oplog, uint32_t genId);
